...
function PatientList() {
    ...
    useEffect(() => {
        axios.get('http://localhost:3001')
            .then(result => {
                // Sort patients by NIC in ascending order
                const sortedPatients = result.data.sort((a, b) => a.nic.localeCompare(b.nic));
                    ...
                    <PageBody>
                        <Stack direction="row" alignItems="center" justifyContent="space-between">
                            ...
                            <Link style={{ textDecoration: 'none' }} to={`/addpatient`}>
                                <AddCircleIcon style={{ cursor: 'pointer', fontSize: 40, color: '#778da9' }} />
                            </Link>
                        </Stack>
